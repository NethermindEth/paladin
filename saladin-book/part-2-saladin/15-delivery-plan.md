...
| **M6** | Sente | ch. 14 §14.3 phases S1–S4 | 6 em | private Soroban contract in a 3-member group + atomic external call to SNoto | ⚠️ S1/S2 and most S3 implementation are in place, including stateful UTXO lifecycle and restart-safe event confirmation. Remaining demo work: run real Sente on three Paladin nodes with one member per node, persistent Testnet configs, all identities funded, and an external SNoto call that uses a real SNoto state ID. S4 hardening, determinism audit, protocol-upgrade drill, and chaos testing remain open. |
