...
type SequencerManager interface {
	...
	HandleChainedTransactionOutcome(ctx context.Context, contractAddress pldtypes.ChainAddress, txID uuid.UUID, receiptType ReceiptType, failureMessage string, revertData pldtypes.HexBytes, onChain pldtypes.OnChainLocation)
}
